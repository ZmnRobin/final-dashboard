...
import { createUser, updateUser, findSingleUser } from '../../_lib/apiCall/admin/userApi';
...
const UserForm: React.FC<UserFormProps> = ({ closeModal, fetchData, isUpdateMode, userId }) => {
  ...
  const onSubmit = async (data: { name: string; phone: string; role: string }) => {
    setLoading(true)
    setError('')
    try {
      if (isUpdateMode) {
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white p-8 rounded-lg w-96">
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {isUpdateMode ? <button disabled={loading} type="submit" className="bg-blue-600 text-white px-4 py-2 rounded-md w-full">
